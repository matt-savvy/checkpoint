...
class CompanyRaceTestCase(TestCase):
    def setUp(self):
        pass

    def test_add_company_create_company_entry(self):
        company = CompanyFactory()
        race = RaceFactory(race_type=Race.RACE_TYPE_COMPANY_RACE)
        race.add_company(company)
        self.assertTrue(CompanyEntry.objects.filter(company=company).filter(race=race).exists())

    def test_add_company_adds_race_entries(self):
        company = CompanyFactory()
        racers = RacerFactory.create_batch(2, company=company)
        race = RaceFactory(race_type=Race.RACE_TYPE_COMPANY_RACE)
        company_entry = race.add_company(company)
        self.assertEqual(RaceEntry.objects.filter(racer__in=racers).count(), 2)
