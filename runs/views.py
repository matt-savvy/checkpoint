...
class CommissionView(AuthorizedRaceOfficalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for run in runs:
            ...
            run_dict['time_picked']= run.utc_time_picked.astimezone(central).strftime('%I:%M %p')
            if run.utc_time_dropped:
                run_dict['time_dropped'] = run.utc_time_dropped.astimezone(central).strftime('%I:%M %p')
            else:
            ...
            due_time = due_time.astimezone(central)
